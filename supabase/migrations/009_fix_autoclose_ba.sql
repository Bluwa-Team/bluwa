-- ══════════════════════════════════════════════════════════════════════════════
-- Migration 009 — Fix auto-close BC/BA : guard lignes sans article MDM
--
-- Bug : si TOUS les purchase_order_items ont article_id IS NULL (article saisi
--       en texte libre), la clause AND poi.article_id IS NOT NULL exclut toutes
--       les lignes → NOT EXISTS retourne TRUE → le BC/BA se ferme immédiatement
--       quel que soit le volume réceptionné.
--
-- Fix : n'exécuter l'auto-close que s'il existe au moins une ligne liée au MDM
--       (article_id IS NOT NULL). Les commandes 100% texte-libre restent ouvertes
--       jusqu'à clôture manuelle.
-- ══════════════════════════════════════════════════════════════════════════════
...
BEGIN
    ...
    -- Guard ajouté : empêche la fermeture immédiate des commandes 100% texte-libre
    -- (tous les article_id IS NULL → le NOT EXISTS précédent retournait TRUE à tort).
