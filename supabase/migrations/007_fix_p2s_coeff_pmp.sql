-- ══════════════════════════════════════════════════════════════════════════════
-- Migration 007 — Correction flux P2S : PMP réel + auto-close par article
--
-- Bug 1 : le trigger utilisait unit_price_ht (prix/unité achat) sans le diviser
--         par coeff_conversion_achat → PMP gonflé de 1000× pour les articles kg→g.
--         Fix : v_unit_price = unit_price_ht / coeff_conversion_achat
--
-- Bug 2 : l'auto-close comparait SUM total de toutes les lignes confondues →
--         un BC pouvait être clôturé si un seul article comblait le total commandé.
--         Fix : vérifier article par article que reçu ≥ commandé (en unités stock).
--         Utilise coeff_conversion_achat (achat→stock), pas coeff_conversion (stock→vente).
-- ══════════════════════════════════════════════════════════════════════════════
...
BEGIN
    ...
    FOR v_item IN
        SELECT gri.article_id,
               ...
               COALESCE(poi.unit_price_ht, 0)            AS unit_price_achat,
    ...
    LOOP
        ...
        -- Prix par unité de stock : ex. 2500 XOF/kg ÷ 1000 (kg→g) = 2.5 XOF/g
        -- Si le BC n'a pas de prix (réception directe), on conserve le PMP actuel.
        v_unit_price := CASE
    ...
    -- Auto-close BC : RECEIVED si toutes les lignes (par article) ont été reçues.
    -- Comparaison en unités de stock : commandé × coeff_conversion_achat vs reçu.
    -- Les lignes sans article_id sont ignorées (elles ne bloquent pas la clôture).
